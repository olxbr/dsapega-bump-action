...
def get_repo_age_metadata_commit_based(default_branch: str = "main") -> float:
    ...
    age_in_months = round(
        (datetime.datetime.now(datetime.timezone.utc) - first_commit_datetime).days / 30,  # fmt: skip
        2,
    )
...
def get_files_by_regex(regex: str = r"(.*?)", dir_to_check: str = "."):
    ...
    for directory in directoryes:
        files_in_dir = get_files_by_regex(dir_to_check=f"{dir_to_check}/{directory}")
...
def get_syft_for_dockerfiles(image_names: list = []):
    ...
    for image in image_names:
        ...
        process = subprocess.run(
            f"syft {image} -o cyclonedx-json", shell=True, capture_output=True
        )
        if process.returncode != 0:
...
def get_syft_sbom(path: str = ".", **kwargs) -> dict:
    ...
    process = subprocess.run(
        f"syft {path} -o cyclonedx-json", shell=True, capture_output=True
    )
    if process.returncode != 0:
...
def compressor(repo: str = "", **kwargs) -> dict:
    ...
    for pkg in packages:
        try:
            ...
            log.warning(
                f"The pkg {pkg['name']}:{pkg['type']} does not have a version or bom-ref"
            )
            pkg_structure = {
                "name": pkg["name"],
                "type": pkg["type"],
...
def process(
    repo: str, token: str, default_branch: str, role: str, verbose: bool, **kwargs
):
    global TOKEN
...
def click_callback(
    repo: str = "", token: str = "", default_branch: str = "main", verbose: bool = True
):
    process(
        repo=repo,
        token=TOKEN,
